...
export default function Sidebar() {
	...
	return (
		<>
			{/* Sidebar Toggle Button (mobile only, appears on top of page) */}
			<button className='lg:hidden fixed top-4 left-4 z-50 bg-primary text-white p-2 rounded-md shadow-md' onClick={() => setIsOpen(true)}>
			...
			<aside
				...
				<button className='lg:hidden absolute top-4 right-4 text-white bg-white/10 hover:bg-white/20 rounded-full p-1 transition' onClick={() => setIsOpen(false)}>
					...
					<nav className='flex flex-col gap-2 mt-6'>
						{[
							...
							<NavLink
								...
								onClick={() => setIsOpen(false)} // auto close on mobile navigation
							>
